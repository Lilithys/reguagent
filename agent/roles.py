...
the response planner while facts are missing. Return waiting when an open question
needs the user; that does not mean the whole investigation failed. This milestone is
investigation, interview and recovery, not final action implementation. Be explicit
about unsupported source versions and remaining review needs.''',
...
the plan. Neither tool approves, accepts, or assigns anything -- a human must still
accept the action separately. Use findings for estimates, not invented final numbers
or assumptions of zero loss.''',
}

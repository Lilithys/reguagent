...
勾选表示实现进度，真实模型端到端验收单独记在 T24。已将此前提前勾选的 T17、T21、T22 恢复为未完成；真实验证暴露任务边界问题后，T12 也恢复为部分完成，已有实现保留。P0 是扩大 LLM 自主性前必须修复的问题；P1 是完整 demo 必需能力；P2 为主线完成后再考虑的扩展。
...
- [ ] **T12 — 案件协调 Agent。**
  **部分完成：** 真实委派和专家续跑已验证；实际任务仍一次覆盖八条要求，没有阶段性 Finding。下一步增加可校验的窄任务/单一交付物合同和未完成工作返回，避免大任务耗尽调查预算。
...
- [ ] **T17 — 参数化成本工具，接入约束重规划。**
  **部分完成：** 底层计算已参数化；案件约束版本、Agent 工具参数和交互 API 尚未贯通。
...
- [ ] **T22 — 接通交互 API 与审计查看。**
  **部分完成：** JSON 查询与待办处理已有，启动/恢复/约束更新等接口及客户端版本校验尚缺。
...
**最新：新增预算已获授权并完成本轮实验。** 实际 **10 次请求/10 次成功响应、33746 tokens、428.588 秒**，无 provider/工具错误，审计和检查点有效。真实发生一个法规专家委派、两次续跑，无完全相同的重复工具请求；但没有 Finding/Question/计划/行动，专家仍在读取过大的八要求任务。新增运行时预算提示及两项测试，全量 **226/226 通过**；提示后的真实一轮仍只读取资料，不能宣称已解决产出问题。T12 恢复为部分完成，优先修复结构化委派范围和阶段性交付；T24 保持未完成。授权尚余 2 次/51.412 秒，未继续消耗。此前自动审批阻塞已解除，不得重复按缺授权处理。详见[本轮报告](research/task_checkpoints_2026-09-12.md)。下方为历史记录，不覆盖本条最新状态。
